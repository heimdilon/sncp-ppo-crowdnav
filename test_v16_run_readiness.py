...
def test_v18_run_readiness_passes_current_repo():
...
def test_v18_run_readiness_flags_stale_notebook(tmp_path):
    notebook = {
        "cells": [
            {
                "cell_type": "code",
                "source": "SAVE_PATH = 'checkpoints/sncp_ppo_v18.pt'\nREPLAY_RATIO = 0.0\n",
            },
            {
                "cell_type": "code",
                "source": "CHECKPOINT = 'checkpoints/sncp_ppo_v18.pt'\ncmd = ['python', 'evaluate_policy_report.py']\n",
            },
        ]
    }
...
def test_colab_notebook_has_preflight_cell_before_v18_training():
    ...
    training_indices = [
        idx for idx, source in enumerate(code_sources) if "SAVE_PATH = 'checkpoints/sncp_ppo_v18.pt'" in source
    ]
    ...
    assert "eval_v18/run_readiness.md" in preflight_cell


def test_colab_persist_cell_downloads_eval_v18_artifact_bundle():
    notebook = json.loads(Path("sncp_ppo_colab.ipynb").read_text(encoding="utf-8"))
    code_sources = [
        _source_text(cell)
        for cell in notebook["cells"]
    ...
    assert "'eval_v18_artifacts'" in persist_cell
    assert "'eval_v18'" in persist_cell
