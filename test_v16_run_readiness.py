...
def test_v22_run_readiness_passes_current_repo():
...
def test_v22_run_readiness_flags_stale_notebook(tmp_path):
    notebook = {
        "cells": [
            {
                "cell_type": "code",
                "source": "SAVE_PATH = 'checkpoints/sncp_ppo_v22.pt'\nREPLAY_RATIO = 0.0\n",
            },
            {
                "cell_type": "code",
                "source": "CHECKPOINT = 'checkpoints/sncp_ppo_v22.pt'\ncmd = ['python', 'evaluate_policy_report.py']\n",
            },
        ]
    }
    (tmp_path / "sncp_ppo_colab.ipynb").write_text(json.dumps(notebook), encoding="utf-8")
    (tmp_path / "run_post_eval.py").write_text("stub\n", encoding="utf-8")
    baseline_dir = tmp_path / "eval_v21"
    baseline_dir.mkdir()
    ...
    assert any("COMFORT_COEFF = 6.0" in note for note in summary.notes)
    assert any("MAX_TIME = 15.0" in note for note in summary.notes)
...
def test_colab_notebook_has_preflight_cell_before_v22_training():
    ...
    training_indices = [
        idx for idx, source in enumerate(code_sources) if "SAVE_PATH = 'checkpoints/sncp_ppo_v22.pt'" in source
    ]
    ...
    assert "eval_v22/run_readiness.md" in preflight_cell


def test_colab_persist_cell_downloads_eval_v22_artifact_bundle():
    notebook = json.loads(Path("sncp_ppo_colab.ipynb").read_text(encoding="utf-8"))
    code_sources = [
        _source_text(cell)
        for cell in notebook["cells"]
    ...
    assert "'eval_v22_artifacts'" in persist_cell
    assert "'eval_v22'" in persist_cell
