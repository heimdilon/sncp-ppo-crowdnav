...
def orca_new_velocity(pos, vel, radius, pref_vel, neighbor_positions, neighbor_velocities, neighbor_radii, max_speed,
    ...
    if len(neighbor_positions) == 0:
        ...
        neighbor_positions = np.asarray(neighbor_positions, dtype=float)
        neighbor_velocities = np.asarray(neighbor_velocities, dtype=float)
        neighbor_radii = np.asarray(neighbor_radii, dtype=float)

        rel_position = neighbor_positions - pos
        rel_velocity = vel - neighbor_velocities
        ...
        combined_radius = radius + neighbor_radii
        ...
        cutoff = outside & (dot1 < 0.0) & (dot1 * dot1 > combined_radius_sq * w_length_sq)
        ...
        if np.any(legs):
            ...
            leg_direction_x[left] = (leg_rel_x[left] * leg[left] - leg_rel_y[left] * leg_radius[left]) / leg_dist_sq[left]
            leg_direction_y[left] = (leg_rel_x[left] * leg_radius[left] + leg_rel_y[left] * leg[left]) / leg_dist_sq[left]

            right = ~left
            leg_direction_x[right] = -(leg_rel_x[right] * leg[right] + leg_rel_y[right] * leg_radius[right]) / leg_dist_sq[right]
            leg_direction_y[right] = -(-leg_rel_x[right] * leg_radius[right] + leg_rel_y[right] * leg[right]) / leg_dist_sq[right]
        ...
        if np.any(colliding):
            ...
            collision_w_length = np.sqrt(collision_w_x * collision_w_x + collision_w_y * collision_w_y)
            ...
            stationary_unit_x[separated] = collision_rel_x[stationary][separated] / rel_length[separated]
            stationary_unit_y[separated] = collision_rel_y[stationary][separated] / rel_length[separated]
            ...
            u_scale = combined_radius[colliding] * inv_time_step - collision_w_length
            u[colliding, 0] = u_scale * unit_w_x
            u[colliding, 1] = u_scale * unit_w_y

        points = vel + responsibility * u
        lines = [{'point': point, 'direction': dir} for point, dir in zip(points, direction)]
