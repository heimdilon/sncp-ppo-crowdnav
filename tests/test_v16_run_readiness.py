...
def test_v35_run_readiness_passes_current_repo():
...
def test_v35_run_readiness_flags_stale_notebook(tmp_path):
    # A pre-v35 notebook (v23..v34 markers) must be flagged: the v35 cells are absent.
    notebook = {
        "cells": [
            {
                "cell_type": "code",
    ...
    assert any("v35 training" in note for note in summary.notes)
    assert any("v35 evaluation" in note for note in summary.notes)
...
def test_colab_persist_cell_downloads_eval_v35_artifact_bundle():
    ...
    assert "'eval_v35_artifacts'" in persist_cell
    assert "'eval_v35'" in persist_cell
