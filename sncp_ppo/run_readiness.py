...
# v34 = v30 (pre-MLP + mean+max pooling) + Beta action distribution (--action_dist beta).
# Reverts v33's multi-head; keeps the v30 recipe (--num_humans_range 10 20,
# --total_steps 2_500_000). Model+PPO single-variable; the Beta variant is auto-detected
# from the checkpoint (absence of actor_logstd). Budget/8m crossing/comfort stay
...
EVALUATION_TOKENS = (
    "CHECKPOINT = 'checkpoints/sncp_ppo_v34.pt'",
    "EVAL_OUT = 'eval_v34'",
    "EVAL_SEED = 100",
    "EVAL_EPISODES = 50",
    "run_post_eval.py",
...
def verify_v16_run_ready(repo_root: str | Path = ".") -> V16RunReadinessSummary:
    ...
    _check_tokens(evaluation_cell, EVALUATION_TOKENS, notes, "v34 evaluation")

    densities = _baseline_densities(repo_root / "eval_v22" / "density_sweep.json", notes)
    if not notes:
        notes.append("PASS: v34 Colab training and evaluation configuration is ready")
