...
def run_expert_episode(env, seed, responsibility=1.0, time_horizon=3.0):
    ...
    while steps < max_steps:
        v, w = expert_action(env, responsibility=responsibility, time_horizon=time_horizon)
        ...
        if terminated or truncated:
            break
    return {
        'robot_node': rn, 'spatial_edges': se, 'temporal_edges': te, 'actions': acts,
        'success': bool(info['success']), 'collision': bool(info['collision']),
        'timeout': bool(info.get('timeout', False)), 'steps': steps,
    }


def collect_dataset(densities, n_per_density, seed, responsibility=1.0, time_horizon=3.0, **regime):
    ...
    for n_humans in densities:
        ...
        for _ in range(n_per_density):
            ep = run_expert_episode(env, seed=seed_cursor, responsibility=responsibility,
        ...
        stats[n_humans] = {
            'episodes': n_per_density, 'success': n_succ, 'collision': n_coll,
            'timeout': n_tout, 'kept': len(ep_lens),
...
def save_dataset(shards, path):
    ...
    for n_humans, shard in shards.items():
        ...
        flat[f'n{n_humans}_episode_lengths'] = np.asarray(shard['episode_lengths'], dtype=np.int64)
...
def _print_stats(stats):
    print(f"{'N':>3} | {'succ':>5} {'coll':>5} {'tout':>5} | {'kept':>5} {'mean_len':>8}")
    print('-' * 44)
    for n_humans in sorted(stats):
        s = stats[n_humans]
        ep = s['episodes']
        print(f"{n_humans:>3} | {s['success']/ep:>5.0%} {s['collision']/ep:>5.0%} "
...
def main(argv=None):
    ...
    parser.add_argument('--n_per_density', type=int, default=50,
    ...
    parser.add_argument('--time_horizon', type=float, default=3.0,
    ...
    shards, stats = collect_dataset(
        args.densities, args.n_per_density, args.seed,
        responsibility=args.responsibility, time_horizon=args.time_horizon,
    )
    _print_stats(stats)
