...
- **v21 = code-ready / Colab run pending — paper reproduction (validation).** Robot 1.0 m/s
  ...
  Notebook/readiness/tests bumped to v21; gate=pass; 132 tests pass. **v18 stays the 0.26 baseline.**
...
| **v21** | **Paper reproduction (validation, not the 0.26 track):** robot `--robot_vpref 1.0` + ORCA peds at parity `--human_vpref_override 1.0` + `--max_time 15`. Tests the speed hypothesis. `robot_vpref`/`human_vpref_override` are new env params threaded through train AND the eval pipeline so they match. | _Colab run pending._ GIF inspection refuted the "ORCA opens gaps" idea (SFM keeps peds ~0.9 m apart, ORCA ~0.6 m — neither knots); the real high-N gap is robot SPEED + geometry (0.6 m robot needs >1.2 m to thread; crowd packs to ~0.9 m; 0.26 m/s too slow). Expect ~90%+ if speed was the gap → validates the implementation. v18 stays the real-robot baseline. |
