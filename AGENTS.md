...
- **v38 = COMPLETE, POSITIVE; training-free action shield is the new candidate on top of v34-fixed-beta.**
  No PPO training was performed. V38 post-processes deterministic v34 actions with a short constant-velocity
  collision-risk shield (`sncp_ppo/action_shield.py`, default horizon 6 steps / 1.5s, safety_margin 0.0).
...
- **GIF inspection (post-v20) refuted the "ORCA opens gaps" premise.** Numeric check (N=10, steady state):
  SFM keeps pedestrians **0.887 m apart (0% overlap)**; ORCA **0.598 m (11% < 0.6)**. Neither crowd knots —
  the env's SFM repulsion already disperses it. So the real high-density bottleneck is **robot SPEED +
...
| **v21** | **Paper reproduction (validation, not the 0.26 track):** robot `--robot_vpref 1.0` + ORCA peds at parity `--human_vpref_override 1.0` + `--max_time 15` + `--human_goal_noise 2.0`. Tests the speed hypothesis. `robot_vpref`/`human_vpref_override`/`human_goal_noise` are new env params threaded through train AND the eval pipeline so they match. **Goal noise** fixes a fidelity gap the user caught: exact-antipodal goals (`goal=-start`) made every path cross (0,0) so the crowd clumped at the center; perturbing goals spreads the crossings like the paper's human trajectories. | _Colab run pending._ GIF inspection refuted the "ORCA opens gaps" idea (SFM keeps peds ~0.9 m apart, ORCA ~0.6 m — neither knots); the real high-N gap is robot SPEED + geometry (0.6 m robot needs >1.2 m to thread; crowd packs to ~0.9 m; 0.26 m/s too slow). Expect ~90%+ if speed was the gap → validates the implementation. v18 stays the real-robot baseline. |
| **v20** | **Single variable:** pedestrian motion model SFM → **ORCA** (pure-Python `crowd_sim/orca.py`, the paper's CrowdSim regime). Env default `human_motion_model='orca'` so train+eval match. Robot stays INVISIBLE (peds avoid each other, not the robot). Everything else = v18. | _Run superseded by inspection:_ Targets high-N collision (v18/v19 proved it's an ENVIRONMENT problem): ORCA stops the crowd knotting at the antipodal-crossing center → navigable gaps. NOT v14's mistake (crowd does NOT yield to robot). ~3× SFM step cost → ~5–6h. Judge vs v18 understanding the crowd model changed (cross-env). |
...
| `test_paper_regime.py` | env `robot_vpref` scales action space + policy; `human_vpref_override` forces parity speed into `humans_vpref`; bare-env defaults stay the 0.26 TurtleBot regime (paper-reproduction config guard) |
