...
- `verify_v16_artifacts.py` checks the completed v16 artifact set: checkpoint, density sweep, comparison,
  training diagnostics, trajectory PNGs, densities, minimum episodes, collapse flag, and replay ratio.
...
1. **Run v16 replay experiment (code pushed in commit `87896bb`).** The vectorized path now has
   ...
   Local verification: `76 passed`; replay smoke exited 0 and logged replay phase shifts plus PPO
   ...
   verifier tests cover the final `eval_v16/` completeness gate.
