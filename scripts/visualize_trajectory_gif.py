...
def run_and_animate(model_path='checkpoints/sncp_ppo.pt', output_gif='trajectory_animation.gif',
    ...
    if not success_found:
        print("Could not find a successful episode in 20 evaluation runs. Cannot animate.")
        return
    ...
    for i in range(env.num_humans):
        h_line, = ax.plot([], [], linestyle='--', color=colors[i % len(colors)], linewidth=1.5, alpha=0.7, label=f'Pedestrian {i+1} Path' if i == 0 else "")
        ...
        c_ellipse = patches.Ellipse(
            ...
            angle=0, 
            color=colors[i % len(colors)], alpha=0.10, fill=True, label='Comfort Space' if i == 0 else ""
        )
