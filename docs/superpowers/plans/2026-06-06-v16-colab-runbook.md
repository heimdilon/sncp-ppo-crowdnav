...
Final status on 2026-06-08: the Colab training subprocess for `logs/training_20260607_131329.csv`
completed cleanly (`Exited with code 0`) after update 1220, and the post-run artifact bundle was
preserved under `eval_v16/`. The saved best checkpoint reported holdout `min=56%`
(`easy=70%`, `hard=62%`, `circle=56%`, collision 18%), but the completed artifact verification is
`fail` because `comparison_vs_v15.md` is `fail`: v16 regressed success at N=1/5/8 and did not improve
N=10. Replay reduced the v15 collapse; it did not produce a deployable improvement.
